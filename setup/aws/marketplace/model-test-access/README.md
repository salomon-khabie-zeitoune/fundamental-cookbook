...
The Fundamental Platform exposes its API through a **private REST API Gateway** with **IAM (SigV4) authentication**. The API Gateway only accepts requests that arrive through a **registered VPC endpoint**, and every method requires an IAM identity allowed to `execute-api:Invoke`. There is no public URL and no API key.

That means you cannot call the model from your laptop directly, and you cannot reach it with a plain port-forward. These two scripts let you test the model **exactly as your own application will** - over IAM-authenticated calls to the private API Gateway - by running from a small jumpbox inside a consumer VPC:
...
- The jumpbox runs **inside a consumer VPC** - one whose `execute-api` interface endpoint is registered with the private API. It never lands in the Fundamental platform VPC.
...
- **A registered consumer VPC.** At least one consumer `execute-api` VPC endpoint must be registered with the private API (deploy `consumer-vpc-endpoint.yaml` and add its endpoint id to the stack's `ConsumerVpcEndpoint<N>Id` parameter). If none is registered, the script stops and tells you how to add one.
- **The jumpbox needs egress** to install the SDK from `dl.cloudsmith.io` (for `fundamental-client[aws-marketplace]`) and `pypi.org` (for `numpy` / `scikit-learn`). In **SSM mode** it also needs SSM connectivity (`ssm` / `ssmmessages` / `ec2messages` interface endpoints, or a NAT path); in **SSH mode** the public subnet's internet gateway covers both. A normal customer application VPC usually already has this.
...
- **A Cloudsmith API token** for the `fundamental-client` index, exported as `CLOUDSMITH_API_KEY`. The SDK is not on public PyPI; it installs from `dl.cloudsmith.io`. **Fundamental issues a unique token to each customer - ask your Fundamental contact for yours.**
